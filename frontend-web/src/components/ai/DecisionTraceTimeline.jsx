          ...
          {items.map((item, index) => {
            ...
            return (
              <li key={`${item.step || item.title}-${index}`} className="relative">
                ...
                {(item.details || []).length ? <ul className="mt-2 space-y-1 text-xs leading-5 text-muted">{item.details.slice(0, 4).map((detail, detailIndex) => <li key={detailIndex}>• {detail}</li>)}</ul> : null}
              </li>
            );
          })}
        </ol>
