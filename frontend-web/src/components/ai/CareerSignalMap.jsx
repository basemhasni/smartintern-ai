    ...
    <AiSectionCard title="Carte des signaux carriere" description="Lecture du profil par domaine technique pour cette offre.">
      {!categories.length ? <AiEmptyState message="La carte des domaines n est pas disponible pour cette analyse." /> : (
        <div className="grid gap-4 lg:grid-cols-2">
          {categories.filter((item) => item && typeof item === 'object').map((item, index) => (
            <article key={item.category || index} className="rounded-stitch border border-line bg-canvas p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {(Array.isArray(item.matchedSkills) ? item.matchedSkills : []).slice(0, 4).map((skill) => <SkillBadge key={skill} tone="success">{skill}</SkillBadge>)}
                {(Array.isArray(item.weakSkills) ? item.weakSkills : []).slice(0, 2).map((skill) => <SkillBadge key={skill} tone="ai">{skill}</SkillBadge>)}
                {(Array.isArray(item.missingSkills) ? item.missingSkills : []).slice(0, 2).map((skill) => <SkillBadge key={skill} tone="danger">{skill}</SkillBadge>)}
              </div>
            </article>
          ))}
        </div>
