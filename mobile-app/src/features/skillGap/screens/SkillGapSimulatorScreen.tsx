...
export function SkillGapSimulatorScreen({ navigation, route }: Props) {
    ...
    return <Screen><View style={styles.toolbar}><IconButton icon="arrow-back" label="Retour" onPress={() => navigation.goBack()}/><Text style={styles.toolbarTitle}>Skill Gap Simulator</Text><View style={styles.spacer}/></View><GlassCard accent style={styles.intro}><SectionHeader title={offer?.title ?? 'Simulation de progression'} subtitle={offer?.company.companyName ?? 'Offre selectionnee'}/><Text style={styles.introText}>Explorez comment les competences acquises et leurs preuves pourraient faire evoluer votre compatibilite. Aucun score ne sera recalcule sur votre appareil.</Text></GlassCard><GlassCard><SectionHeader title="Mode de simulation" subtitle="Choisissez le niveau des hypotheses utilisees par le moteur IA"/><View style={styles.selector}><SimulationModeSelector value={mode} onChange={(value) => { simulator.clearError(); setMode(value); }}/></View></GlassCard>{!latestCv ? <StatusMessage tone="info" message="Ajoutez et faites analyser votre CV depuis le profil avant de lancer une simulation pertinente."/> : null}{simulator.error ? <StatusMessage tone="error" message={simulator.error}/> : null}<GradientButton icon="sparkles-outline" label={result ? 'Relancer cette simulation' : 'Simuler ma progression'} loading={simulator.isSimulating} disabled={!latestCv} onPress={() => void simulator.simulate(route.params.offerId, mode)}/>{simulatedAt ? <Text style={styles.timestamp}>Derniere simulation: {new Date(simulatedAt).toLocaleString()}</Text> : null}{result ? <View style={styles.results}><ScorePotentialCard result={result}/><HighImpactGapsCard items={result.highImpactGaps}/><SingleSkillSimulationsCard items={result.singleSkillSimulations}/><CombinationSimulationsCard items={result.combinationSimulations}/><RecommendedPathTimeline steps={result.recommendedPath}/><RecommendedProjectsCard projects={result.recommendedProjects}/><SimulationWarningsCard warnings={result.warnings} assumptions={result.assumptions} caps={result.scoreCapsApplied}/></View> : <GlassCard><Text style={styles.empty}>La simulation reste a lancer pour ce mode. Le traitement peut prendre quelques secondes car il passe par le backend et le service IA.</Text></GlassCard>}</Screen>;
}
