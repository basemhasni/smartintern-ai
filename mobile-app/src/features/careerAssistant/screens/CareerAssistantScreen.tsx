...
export function CareerAssistantScreen({ navigation, route }: Props) {
  ...
  const offersState = useOffers(); const { latestCv } = useStudentDashboard(); const assistant = useCareerAssistant();
  ...
  return <Screen><View style={styles.toolbar}><IconButton icon="arrow-back" label="Retour" onPress={() => navigation.goBack()} /><Text style={styles.toolbarTitle}>Assistant carriere</Text><View style={styles.spacer} /></View>{offers.length ? <View style={styles.selector}><Text style={styles.sectionTitle}>Choisir une offre</Text><OfferAnalysisSelector offers={offers} selectedId={offerId} onSelect={(id) => { assistant.clearError(); setOfferId(id); }} /></View> : <GlassCard><EmptyState icon="briefcase-outline" title="Aucune offre disponible" message="Une offre publiee est necessaire pour generer des conseils personnalises." /></GlassCard>}{offer ? <GlassCard accent><Text style={styles.eyebrow}>Conseils pour</Text><Text style={styles.offerTitle}>{offer.title}</Text><Text style={styles.company}>{offer.company.companyName}</Text><Text style={styles.intro}>Career Assistant V2 s appuie sur votre CV analyse, le matching et les sources autorisees. Ses conseils ne garantissent pas un recrutement.</Text></GlassCard> : null}{!latestCv ? <StatusMessage tone="info" message="Ajoutez et faites analyser votre CV depuis le profil pour recevoir des recommandations personnalisees." /> : null}{assistant.error ? <StatusMessage tone="error" message={assistant.error} /> : null}{offerId && latestCv ? <GradientButton icon="compass-outline" label={advice ? 'Actualiser mes recommandations' : 'Generer mes recommandations'} loading={assistant.isGenerating} onPress={() => void assistant.generate(offerId)} /> : null}{advice ? <View style={styles.results}><ReadinessCard advice={advice} generatedAt={offerId ? assistant.getGeneratedAt(offerId) : undefined} />{advice.directAnswer ? <StatusMessage tone="info" message={advice.directAnswer} /> : null}{advice.priorityFocus.length ? <PriorityFocusCard items={advice.priorityFocus} /> : null}{advice.criticalGaps.length ? <CriticalGapsCard items={advice.criticalGaps} /> : null}{advice.learningRoadmap.length ? <LearningRoadmapTimeline items={advice.learningRoadmap} /> : null}{advice.cvImprovementTips.length ? <CvImprovementCard tips={advice.cvImprovementTips} /> : null}{advice.recommendedProjects.length ? <CareerProjectsCard projects={advice.recommendedProjects} /> : null}{advice.interviewPreparationTips.length ? <InterviewPreparationCard tips={advice.interviewPreparationTips} /> : null}{advice.sources.length ? <CareerSourcesCard sources={advice.sources} /> : null}{[...advice.warnings, ...advice.ragWarnings].map((warning) => <StatusMessage key={warning} tone="warning" message={warning} />)}</View> : offerId && latestCv ? <GlassCard><EmptyState icon="compass-outline" title="Recommandations non generees" message="Lancez une analyse uniquement lorsque vous souhaitez actualiser votre plan de progression." /></GlassCard> : null}{offerId ? <GradientButton icon="document-text-outline" label="Rediger ma lettre de motivation" onPress={() => navigation.navigate('MotivationLetterGenerator', { offerId })} variant="secondary" /> : null}{offerId && latestCv ? <CareerQuestionForm loading={assistant.isSubmittingQuestion} onSubmit={(question) => assistant.ask(offerId, question)} /> : null}<CareerAnswerHistory answers={answers} onClear={() => offerId && assistant.clearAnswers(offerId)} /></Screen>;
}
