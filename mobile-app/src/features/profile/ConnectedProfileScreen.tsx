...
export function ConnectedProfileScreen() {
  ...
  const rows = [
    { icon: 'mail-outline', label: 'Adresse email', value: user?.email ?? 'Non renseigné' },
    { icon: 'shield-checkmark-outline', label: 'Type de compte', value: user?.role === 'STUDENT' ? 'Étudiant' : user?.role ?? 'Étudiant' },
    { icon: 'checkmark-circle-outline', label: 'Session', value: 'Sécurisée et active' },
  ] as const;

  return <Screen eyebrow="Compte" title="Mon profil" subtitle="Gérez votre identité et vos préférences d’affichage."><GlassCard accent style={styles.identity}><View style={styles.avatarWrap}><View style={styles.avatar}><Text style={styles.avatarText}>{getUserInitials(user)}</Text></View><View style={styles.verified}><Ionicons color={theme.colors.white} name="checkmark" size={13} /></View></View><Text style={styles.name}>{displayName}</Text><Text style={styles.email}>{user?.email}</Text><AppBadge icon="school-outline" label="Profil étudiant" tone="violet" /><View style={styles.completion}><View style={styles.completionHeader}><Text style={styles.completionLabel}>Profil complété</Text><Text style={styles.completionValue}>72%</Text></View><View style={styles.track}><View style={styles.progress} /></View></View></GlassCard><Text style={styles.sectionTitle}>Informations du compte</Text><GlassCard style={styles.details}>{rows.map((row, index) => <View key={row.label}><View style={styles.row}><View style={styles.rowIcon}><Ionicons color={theme.colors.primary} name={row.icon} size={19} /></View><View style={styles.copy}><Text style={styles.label}>{row.label}</Text><Text numberOfLines={1} style={styles.value}>{row.value}</Text></View></View>{index < rows.length - 1 ? <View style={styles.divider} /> : null}</View>)}</GlassCard><Text style={styles.sectionTitle}>Apparence</Text><View accessibilityRole="radiogroup" style={styles.themeControl}>{themeOptions.map((option) => { const selected = preference === option.value; return <Pressable accessibilityRole="radio" accessibilityState={{ selected }} key={option.value} onPress={() => setPreference(option.value)} style={[styles.themeOption, selected && styles.themeOptionActive]}><Ionicons color={selected ? theme.colors.white : theme.colors.textSecondary} name={option.icon} size={18} /><Text style={[styles.themeOptionText, selected && styles.themeOptionTextActive]}>{option.label}</Text></Pressable>; })}</View><Pressable disabled={isLoading} onPress={() => void logout()} style={({ pressed }) => [styles.logout, pressed && styles.pressed, isLoading && styles.disabled]}><Ionicons color={theme.colors.danger} name="log-out-outline" size={20} /><Text style={styles.logoutText}>{isLoading ? 'Déconnexion...' : 'Se déconnecter'}</Text></Pressable><Text style={styles.version}>SmartIntern AI · Version mobile 1.0</Text></Screen>;
}
...
const createStyles = (theme: AppTheme) => StyleSheet.create({
  ...
  progress: { width: '72%', height: '100%', backgroundColor: theme.colors.emerald, borderRadius: 3 },
