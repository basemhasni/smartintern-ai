...
def _safe_known_skills(candidate_skills: list[str], matched_skills: list[str]) -> list[str]:
    candidate_lookup = {skill.lower(): skill for skill in _dedupe_skills(candidate_skills)}
    safe_skills = []

    for skill in _dedupe_skills(matched_skills):
...
def generate_motivation_letter(payload) -> dict:
    ...
    first_name = _clean(student.firstName) or "l'etudiant"
    last_name = _clean(student.lastName) or ""
    full_name = f"{first_name} {last_name}".strip()
    education = _clean(student.educationLevel)
    target_job = _clean(student.targetJob)
    bio = _clean(student.bio)
    company_name = _clean(company.companyName) or "votre entreprise"
    sector = _clean(company.sector)
    offer_title = _clean(offer.title) or "votre offre de stage"
    location = _clean(offer.location)
    duration = _clean(offer.duration)
    matched_skills = matching.matchedSkills if matching else []
    safe_skills = _safe_known_skills(payload.candidateSkills, matched_skills)
    skills_sentence = _join_skills(safe_skills)
    ...
    paragraphs.extend(["", f"Veuillez agreer, Madame, Monsieur, l'expression de mes salutations distinguees.", full_name])
