...
def _missing_optional_skills(candidate_skills: list[str], matched_skills: list[str], optional_skills: list[str]) -> list[str]:
...
def _build_profile_summary(score: int, offer_title: str) -> str:
    if score >= 80:
...
def _build_action_plan(skills_to_improve: list[dict]) -> list[dict]:
...
def generate_career_advice(payload) -> dict:
    ...
    candidate_skills = _normalize_list(payload.candidateSkills)
    matched_skills = _normalize_list(matching.matchedSkills)
    missing_skills = _normalize_list(matching.missingSkills)
    optional_skills = _normalize_list(offer.optionalSkills)
    optional_missing_skills = _missing_optional_skills(candidate_skills, matched_skills, optional_skills)
    ...
    action_plan = _build_action_plan(skills_to_improve)
    ...
    return {
        "profileSummary": _build_profile_summary(matching.score, offer.title),
